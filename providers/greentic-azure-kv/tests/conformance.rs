...
impl AzureClient {
    ...
    fn uri_for(&self, key: &str) -> SecretUri {
        ...
        let scope = Scope::new("int".into(), "azure".into(), None).unwrap();
...
async fn ensure_bearer_token() -> Result<()> {
    ...
    let credential: Box<dyn TokenCredential> = if !client_secret.is_empty() {
        let tenant = std::env::var("AZURE_TENANT_ID")?;
        let client = std::env::var("AZURE_CLIENT_ID")?;
        Box::new(ClientSecretCredential::new(tenant, client, client_secret)?)
    } else {
        Box::new(DefaultAzureCredential::default())
    };
