...
mod host {
    ...
    use aws_sdk_secretsmanager::{config::Region, error::SdkError, Client};
    ...
    static CTX: OnceCell<AwsCtx> = OnceCell::new();
    static FAKE: Lazy<bool> =
        Lazy::new(|| std::env::var("GREENTIC_AWS_FAKE").is_ok() || cfg!(test));
    static STORE: Lazy<Mutex<HashMap<String, Vec<u8>>>> = Lazy::new(|| Mutex::new(HashMap::new()));

    fn client_from_config(cfg: Config) -> Result<AwsCtx, String> {
        ...
        let client = rt.block_on(async move {
            ...
            let mut sm_cfg = aws_sdk_secretsmanager::config::Builder::from(&shared);
            if let Some(endpoint) = cfg.endpoint {
                sm_cfg = sm_cfg.endpoint_url(endpoint);
            }
            Ok::<Client, String>(Client::from_conf(sm_cfg.build()))
        })?;
        Ok(AwsCtx { rt, client })
    }

    fn ctx(cfg_json: &[u8]) -> Result<&'static AwsCtx, String> {
        CTX.get_or_try_init(|| {
            let cfg: Config =
    ...
    fn handle_put(ctx: &AwsCtx, key: String, value: Value) -> Vec<u8> {
        ...
        let res = ctx.rt.block_on(async move {
            let put = client
                .put_secret_value()
                .secret_id(key.clone())
                .secret_binary(bytes.clone().into())
                .send()
                .await;
            if let Err(SdkError::ServiceError(svc_err)) = &put {
                if svc_err.err().is_resource_not_found_exception() {
                    client
                        .create_secret()
                        .name(key.clone())
                        .secret_binary(bytes.into())
                        .send()
                        .await
                        .map(|_| ())
                        .map_err(|e| format!("{e}"))?;
    ...
    fn handle_get(ctx: &AwsCtx, key: String) -> Vec<u8> {
        let client = ctx.client.clone();
        let res = ctx.rt.block_on(async move {
            let out = client
                .get_secret_value()
                ...
                .await
                .map_err(|e| format!("{e}"))?;
            if let Some(bytes) = out.secret_binary {
                Ok(bytes.into_inner())
            } else if let Some(s) = out.secret_string {
                Ok(s.into_bytes())
            } else {
                Err("empty secret".to_string())
            }
        });
        match res {
    ...
    fn handle_delete(ctx: &AwsCtx, key: String) -> Vec<u8> {
        let client = ctx.client.clone();
        let res = ctx.rt.block_on(async move {
            client
                .delete_secret()
...
mod wasm {
    ...
    #[allow(dead_code)]
    pub(super) fn handle_inmemory(op: String, parsed: serde_json::Value) -> Vec<u8> {
        match op.as_str() {
            "put" => {
                let input: PutInput = match serde_json::from_value(parsed) {
                    Ok(v) => v,
...
mod tests {
    ...
    fn roundtrip_put_get_delete() {
        std::env::set_var("GREENTIC_AWS_FAKE", "1");
        ...
        let validated = Provider::validate_config(b"{}".to_vec());
        assert_eq!(
            as_json(validated).get("status").and_then(|v| v.as_str()),
            Some("ok")
        );

        // In tests, this will hit the wasm shim (in-memory) on wasm32 and will
        // use real AWS otherwise; real AWS requires credentials in env.
        ...
        let decoded = general_purpose::STANDARD
            .decode(val_b64)
            .unwrap_or_default();
        // If running against real AWS without setup, the value may differ; the test ensures no panic.
